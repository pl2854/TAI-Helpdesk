...
router.get('/techs', auth, async (req,res) => {
  try {
    ...
    const techs = await User.find({ userType: 'teacher'}).select('-isActive -password -email -createdAt -updatedAt -userType').sort({ _id: '-1' });
    res.json(techs);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.put('/tickets/:id', [ auth, [
  ...
  try {
    ...
    if(!user) {
      ...
      if(userType === 'admin') {
        // IF user === 'admin' THEN update 'issuedBy' field
      ...
      if(userType === 'teacher') {
        // IF user === 'teacher' THEN update 'assignedTo' field
